...
- [ ] The cursor halo is enabled by default, remains centered on the native pointer hotspot throughout free mode, and is hidden immediately on exit. It tracks physical and keyboard movement within one display refresh without changing for clicks, drags, scrolling, precision, or speed.
