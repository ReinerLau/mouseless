...
BUILD_DIR=$(mktemp -d "${TMPDIR:-/tmp}/mouseless-scroll-smoke.XXXXXX")
...
command -v swiftc >/dev/null || {
  echo "swiftc is required to run the scroll smoke test." >&2
  exit 1
}
...
swiftc "$SCRIPT_DIR/scroll-smoke-test.swift" -framework CoreGraphics -o "$BUILD_DIR/scroll-smoke-test"
"$BUILD_DIR/scroll-smoke-test"
