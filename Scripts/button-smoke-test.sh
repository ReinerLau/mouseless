...
BUILD_DIR=$(mktemp -d "${TMPDIR:-/tmp}/mouseless-button-smoke.XXXXXX")
...
command -v swiftc >/dev/null || {
  echo "swiftc is required to run the button smoke test." >&2
  exit 1
}
...
swiftc "$SCRIPT_DIR/button-smoke-test.swift" -framework CoreGraphics -o "$BUILD_DIR/button-smoke-test"
"$BUILD_DIR/button-smoke-test"
